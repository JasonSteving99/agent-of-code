...
def find_antinodes(ant1: Tuple[int, int], ant2: Tuple[int, int], grid: List[List[str]]) -> Set[Tuple[int, int]]:
    ...
    for direction in [-1, 1]:
        x_antinode = x1 + (2 * dx) / 3.0 if direction == 1 else x2 + (dx) / 3.0
        y_antinode = y1 + (2 * dy) / 3.0 if direction == 1 else y2 + (dy) / 3.0

        x_antinode_int = int(round(x_antinode))
        y_antinode_int = int(round(y_antinode))

        if is_in_bounds((x_antinode_int, y_antinode_int), grid):
            antinodes.add((x_antinode_int, y_antinode_int))
